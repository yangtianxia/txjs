...
type ValidatorRuleValue<Value, Template> = Value extends boolean
  ? Template | { value?: Value }
  : { value: Value }

type ValidatorRuleOption<Value, Template, Trigger> =
  | ValidatorRuleValue<Value, Template>
  | {
      /** 触发事件 */
      trigger?: Trigger
      /** 消息模版 */
...
type ValidatorRule<Trigger, Custom, VO, MO> = Partial<
  ExtractValidatorRules<Trigger, VO, MO>
> & {
  /** 数据类型 */
  type?: FieldType
  /** 标签文本 */
...
type UnwrapSchema<T, Trigger, Custom, VO, MO> = Partial<
  Record<keyof T, ValidatorRule<Trigger, Custom, VO, MO>>
>
...
type RuleObject<T, Trigger, Custom, MsgType> = Record<
  keyof T,
...
> {
  ...
  #generate<K>(key: K, rule: ValidatorRule<Trigger, Custom, VO, MO>) {
    ...
    for (const name in rest) {
      ...
      rules.push(
        this.#convert(name, rule, validation, {
          type,
          trigger,
          param: value,
          message:
            this.errorPhase === 'pre' ? messageCallback() : messageCallback,
        })
      )
    }
    ...
    if (custom) {
      ...
      for (let i = 0, len = customs.length; i < len; i++) {
        ...
        if (!isFunction(validator)) {
          throwError(
            `"${key}" 'custom${[i]}.validator' value must be a function.`
          )
        }
        rules.push(
          this.#convertCustom(`${key}-${i}`, rule, validator, {
            type,
            label,
            trigger,
          })
  ...
  #formatMessage(options: {
    ...
    const { name, param, label, message, template } = options
  ...
  #convert(
    ...
    const { param, type, message } = partial
    ...
    return {
      ...
      validator: ((_, value) => {
        return new Promise<void>((resolve, reject) => {
          if (
            (isNil(rule.required) && isEmptyFieldValue(type, value)) ||
            param === false
          ) {
            resolve()
          } else if (
            !validators.every((validator) => validator(value, param, type))
          ) {
            reject(
              new Error(
                formatTplByValue(
          ...
          if (isNil(rule.required) && isEmptyFieldValue(partial.type, value)) {
            ...
            try {
              ...
              if (isPromise(promify)) {
                promify.then(resolve).catch((error: Error) => {
